...
> Generated 2026-06-10 from a read-only audit of the full repo (direct reading of core wiring + three parallel audit passes: security, performance/quality, data-layer/CI). Every claim cites the file it came from; items based on inference rather than direct reading are marked **(inferred)**.
>
...
>
> **Coverage caveat:** two of five audit agents (deep route-history and deep AI-layer passes) were cut off by a session limit. Their areas are covered by direct verification (nav/sitemap/redirect analysis, AI wiring spot-checks) plus overlap from the other three passes — but per-route git history dates and a line-by-line review of all 19 `src/lib/ai/*` modules were not completed. Flagged where it matters.
...
| S-6 | Medium | Custom markdown renderer allows `javascript:` URLs → stored XSS | `src/lib/markdown.ts:94-115`, rendered via `dangerouslySetInnerHTML` in `src/app/blog/[slug]/page.tsx:361` + other blog/experiencer pages | `[x](javascript:alert(1))` in AI-generated or admin-entered content executes; sanitizer strips tags/handlers but not URL schemes | Whitelist schemes (http/https/mailto/relative), or swap to rehype-sanitize/DOMPurify |
...
| A-6 | Low | Legacy Genkit flow + n8n-webhook chat path coexist with the live OpenAI chat | `src/ai/flows/compassionate-chatbot-answers.ts`, `src/app/actions.ts:73`, `NEXT_PUBLIC_*_WEBHOOK_URL` in `apphosting.yaml` | Three parallel chat backends; dependency weight (genkit) for one flow | After route cleanup (D-1), decide: keep Genkit or fold the flow into the OpenAI path; drop unused webhook env vars |
...
|---|---|---|---|---|
| D-1 | **High** | Route folders: `chat-2`, `chat-test`, `search`, `search2`, `about-new`, `home-v2`, `video-2025`, `header-preview`, `logo-preview`, `uap/people`, `chat/page.tsx` (unreachable behind 301), `search3/dev/` | No inbound links; sitemap uses `/video/[id]` not `/video-2025`; `/chat` 301s in `next.config.ts:39`. Only inbound ref to `chat-2` is from `experiencers` (itself semi-orphaned, D-3) | Delete (git-recoverable). `home-new`: relocate shared modules first (A-3) |
| D-2 | **High** | Dead components: `src/components/site-header.tsx` (971 lines, zero imports), `main-nav.tsx`, `search-result-card-v4.tsx` | Import grep | Delete |
...
### Phase 1 — Security hardening (≈1–2 days) · fixes S-1…S-11
The only phase addressing money-losing/exploitable-today issues.
1. Shared per-IP rate limiter (generalize `api/questions/custom/route.ts:21-40`) applied to: `chat-compassionate`, `uap/chat`, `search3`, `uap/search`, `quiz-lead`, `contact`, `ces-feedback`.
2. Auth on `email/manage-subs` POST (token-verified, mirroring GET).
3. Fix `IS_DEBUG_MODE`: `NODE_ENV !== 'production' && === 'true'` everywhere it appears (9 routes).
4. Remove `?secret=` acceptance; header-only CRON auth. **Coordinated change: update Oracle crontab curls in the same window.**
5. Delete debug logging in `proxy.ts`; URL-scheme whitelist in `lib/markdown.ts`; `<` escape in JSON-LD; remove referer path in `uap/analytics`.
- *Verify:* curl each endpoint un/auth'd + over-limit; run one manual crontab tick against prod after the secret change.

### Phase 2 — CI safety net (≈half a day) · fixes T-1, T-3
`ci.yml` on PR + push-to-main: `npm ci` → `tsc --noEmit` (start `continue-on-error: true`) → new ESLint flat config → `next build` with dummy env. Branch protection on `main` requiring the check — this is what actually gates the auto-deploy. Do this **before** the cleanup phases so they land through a gate.
- *Verify:* a PR with a deliberate type error shows the (non-blocking) failure; a broken import blocks merge.

### Phase 3 — Dead code & route cleanup (≈1–2 days) · fixes D-1…D-8, A-3, A-5 — **destructive, see §4**
1. Relocate `home-new` shared modules (A-3) → then delete dead route folders (D-1).
2. Delete dead components (D-2), tracked junk (D-7), disabled workflow stubs (D-6), `execution/` (A-5).
3. `scripts/` → move one-offs to `scripts/archive/` (D-5); add `scripts/README.md`.
4. Uninstall `firebase`, `firebase-admin`, `howler`, `@types/howler` (D-4).
5. Drop now-unused `NEXT_PUBLIC_*_WEBHOOK_URL` vars from `apphosting.yaml` (after confirming only dead chat pages used them).
6. Untracked junk (D-8): owner reviews list, then delete; extend `.gitignore`.
- *Verify:* `next build` green; click through nav; one Oracle tick (scanner-process) confirmed working post-archive.
...
**Deliberately deferred (post-launch):** CSP nonces (S-8), monster-file refactors (Q-4), Genkit/n8n consolidation decision (A-6), `experiencers` page fate (D-3 — needs your call on whether that marketing page has a future).
...
|---|---|---|
| Delete ~12 route folders, 3 dead components, tracked junk files, 17 workflow stubs, `execution/` | 3 | ✅ Fully (git history) |
| `npm uninstall firebase firebase-admin howler @types/howler` | 3 | ✅ (reinstall) |
...
| Drop `NEXT_PUBLIC_*_WEBHOOK_URL` from `apphosting.yaml` | 3 | ✅ (values restorable from Firebase console history; the n8n workflows themselves are untouched) |
