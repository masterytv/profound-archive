...
- **Supadata Quota vs Rate Limit:** Supadata's 429 errors have TWO causes: (1) per-second rate limit burst (retryable with exponential backoff) and (2) monthly credit quota exhaustion (non-retryable — stop pipeline immediately). `subtitles.ts` parses the 429 body for `"error":"limit-exceeded"` to distinguish them. NEVER retry on quota exhaustion — it burns thousands of pointless requests. The pipeline halts and preserves remaining queue items as `pending`. Current plan: Mega (30K credits/mo, $47).
- **Oracle Cloud Always-On Worker:** UAP video processing runs 24/7 on an Oracle Cloud Free Tier VM (`profound-worker`, IP `150.230.166.48`). It executes `scripts/rapid-process.ts` with CONCURRENCY=3 via pm2, pulling from `uap_scan_queue`. The GHA `uap-scanner-process.yml` cron (3/tick, every 10 min) serves as a fallback. Both systems share the same queue — the `status` column prevents duplicate processing. When modifying `rapid-process.ts`, `intake-uap.ts`, or any pipeline dependency, you MUST also deploy to the Oracle worker: `ssh ubuntu@150.230.166.48 → cd ~/profound-archive && git pull && pm2 restart profound-worker`. Full details in `directives/ARCHITECTURE.md` §10.2.