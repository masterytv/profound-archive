...
export function ChannelCard({ channel }: { channel: ChannelStats }) {
    return (
        <Link
            ...
            className="group relative flex flex-col items-center overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-700/60 bg-white dark:bg-slate-800/80 shadow-sm hover:shadow-xl hover:border-blue-200/60 dark:hover:border-blue-700/60 transition-all duration-300 p-6 text-center"
        >
            {/* Channel Logo */}
            <div className="relative mb-4">
                {channel.avatar_url ? (
                    <div className="relative w-20 h-20 rounded-full overflow-hidden ring-3 ring-slate-100 dark:ring-slate-700 group-hover:ring-blue-100 dark:group-hover:ring-blue-800 transition-all duration-300 shadow-md group-hover:shadow-lg">
                        <Image
            ...
            {/* Channel Name */}
            <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2 mb-1">
            ...
            {/* Stats Grid */}
            <div className="w-full grid grid-cols-3 gap-1 rounded-xl bg-slate-50 dark:bg-slate-700/60 p-3">
                <div className="flex flex-col items-center">
                    <div className="flex items-center gap-1 text-slate-400 mb-0.5">
                        <Film className="w-3 h-3" />
                    </div>
                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{channel.video_count}</span>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Videos</span>
                </div>
                <div className="flex flex-col items-center border-x border-slate-200/60 dark:border-slate-700/40">
                    <div className="flex items-center gap-1 text-slate-400 mb-0.5">
                        <Eye className="w-3 h-3" />
                    </div>
                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{formatNumber(channel.total_views)}</span>
