...
export async function processVideoIntake(
    ...
    try {
        ...
        if (!captionFetch.success) {
            // Transient failures should be retried, genuine no-captions should not
            const captionStatus: IntakeStatus = captionFetch.retryable
                ? 'caption_fetch_failed'
                : 'no_captions';
            logStep('Fetch Captions', 'failed',
                `${captionFetch.failureReason}: ${captionFetch.message || 'No details'}`);
            await insertVideoRecord(supabase, videoId, metadata, null, 'insufficient_info', captionStatus);
            return {
