...
async function getCrossDomainData(): Promise<CrossDomainResult | null> {
  ...
  // 1. Cache-first: readCrossDomainCache ignores poisoned all-zero rows.
  const cached = await readCrossDomainCache(supabase);
  if (cached) return cached;

  // 2. Recompute from source tables (RPC-first, paginated-scan fallback) and
  //    re-warm the cache. computeCrossDomainResult never returns zeroed data
  //    and writeCrossDomainCache never caches it.
  const computePromise = (async () => {
    const result = await computeCrossDomainResult(supabase);
    if (result) await writeCrossDomainCache(supabase, result);
    return result;
  })();

  // If the render times out below, the response is sent and Cloud Run
  // throttles the instance — a dangling fire-and-forget promise can die
  // before the cache warms. after() keeps the runtime alive until the
  // compute + cache write actually finish.
  after(() => computePromise.catch(() => undefined));

  try {
    // 12s budget: the RPC path returns in ~2s, but until the RPC migration is
    // applied the fallback pages through ~13k rows. On timeout the page shows
    // its "unable to load" state while after() lets the compute finish and
    // warm the cache for the next render.
    return await withTimeout(computePromise, 12000, null);
  } catch (error) {
