    ...
    try {
        ...
        if (verifyMode && targetVideoId) {
            const { data: analysis, error } = await supabase
                .from('nde_analysis')
                .select('video_id, experience_type, experience_type_confidence, core_elements, trigger_category, overall_tone, intensity_rating, content_safety')
        ...
        if (targetVideoId) {
            ...
            const { data: videos, error } = await supabase
                .rpc('get_unanalyzed_core_elements_videos', { batch_limit: limit });
        ...
        const processPromises = videosToProcess.map(async (video) => {
            try {
                ...
                const analysisResult = await analyzeCoreElements(video.subtitles_punctuated);

                // Determine values to save
                let experienceType: string;
                let experienceTypeConfidence: number;
                let coreElements: any;
                let triggerCategory: string;
                let triggerDescription: string;
                let overallTone: string;
                let intensityRating: number;
                ...
                if (analysisResult) {
                    ...
                    coreElements = { error: 'AI analysis returned null', timestamp: new Date().toISOString() };
                ...
                if (dbOp.error) {
                    ...
                    return { videoId: video.videoId, status: 'success', experienceType };
                }
            } catch (err: any) {
