    ...
    try {
        ...
        if (verifyMode && targetVideoId) {
            const { data: analysis, error } = await supabase
                .from('nde_analysis')
                .select('video_id, phenomenology, entities')
        ...
        if (targetVideoId) {
            ...
            const { data: videos, error } = await supabase
                .rpc('get_unanalyzed_phenomenology_videos', { batch_limit: limit });
        ...
        const processPromises = videosToProcess.map(async (video) => {
            try {
                ...
                const analysisResult = await analyzePhenomenologyEntities(video.subtitles_punctuated);

                // Determine values to save
                let phenomenology: any;
                let entities: any;
                ...
                if (dbOp.error) {
                    ...
                    return { videoId: video.videoId, status: 'success', entityCount, vividness };
                }
            } catch (err: any) {
