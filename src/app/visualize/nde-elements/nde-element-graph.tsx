...
interface ElementEdge {
  source: string | ElementNode;
  target: string | ElementNode;
  weight: number;
  weightPct: number;
}
...
const CATEGORIES: Record<string, { label: string; color: string }> = {
  perceptual: { label: 'Perceptual', color: '#60a5fa' },
  encounter:  { label: 'Entity Encounter', color: '#f472b6' },
  cognitive:  { label: 'Cognitive', color: '#34d399' },
  boundary:   { label: 'Boundary', color: '#f87171' },
  affective:  { label: 'Affective', color: '#86efac' },
};

// ─── Component ──────────────────────────────────────────────────────────────

export function NdeElementGraph() {
  ...
  useEffect(() => {
    fetch('/api/viz/nde-elements')
      ...
      .catch(err => {
        console.error('[nde-element-graph] fetch error:', err);
  ...
  const handleEngineStop = useCallback(() => {
    ...
    // Auto-fit: zoom camera to frame all nodes with 20% padding
    graphRef.current.zoomToFit(600, 60);

    // Enable auto-rotation after fit (desktop only)
    try {
  ...
  const filteredData = useMemo(() => {
    ...
    const enabledNodes = graphData.nodes
      ...
      .map(n => ({
        ...n,
        val: 1 + (n.frequencyPct / 100) * 3,
      }));
    const enabledNodeIds = new Set(enabledNodes.map(n => n.id));

    // Filter edges by category membership AND strength threshold
    ...
    // Remove orphan nodes (no remaining connections)
  ...
  const handleNodeClick = useCallback((node: ElementNode, event: MouseEvent) => {
    ...
    setTooltip({
      ...
      title: node.label,
      stats: [
        { label: 'Present in', value: `${node.frequencyPct}% of experiences` },
  ...
  const handleLinkClick = useCallback((link: any, event: MouseEvent) => {
    ...
    setTooltip({
      ...
      stats: [
        { label: 'Co-occur in', value: `${link.weightPct}% of experiences` },
        { label: 'Together in', value: `${link.weight.toLocaleString()} experiences` },
      ],
      description: 'These two elements appear together in the same near-death experience.',
      x: event.clientX,
      y: event.clientY,
    });
  }, []);
  ...
  const nodeThreeObject = useCallback((node: ElementNode) => {
    ...
    // Measure text to size canvas exactly
    ...
    // Scale to world units — keep labels small and readable
    const spriteScale = 0.035;
    sprite.scale.set(
    ...
    // Position above the built-in sphere
  ...
  return (
    <VizPageShell
      title="NDE Element Connections"
      subtitle={graphData ? `How ${graphData.metadata.totalExperiences.toLocaleString()} near-death experiences connect` : undefined}
      domain="nde"
    ...
    >
      {error ? (
        ...
        <div ref={containerRef} className="w-full h-full relative">
          ...
          {/* ─── Info Overlay — removed, now in sidebar ─── */}
