...
| `merged/<domain>/section-NN.md` | 18 section digests, about 800 videos each, same six sections; written by Claude subagents to the merge-digests.mjs specification and used as the source of the narrative atlas |
| `merged-openai/<domain>/section-NN.md` | The same 18 sections regenerated by `merge-digests.mjs` on gpt-5.6-luna (about $0.28); shorter and citing fewer IDs than `merged/`, kept as the reproducible pipeline output |
...
Observed costs on 2026-09-08: the batch digest run was 6.33M prompt and 0.78M completion tokens on gpt-5.6-luna, about $2.21, with no fallbacks. The level-1 merge on gpt-5.6-luna cost about $0.28 for all 18 sections once credits were added; it first had to be done by Claude subagents because the OpenAI balance ran out after the digest run.
...
- `merged/` holds the Claude-subagent sections the narrative was built from; `merged-openai/` holds the script-generated equivalents. A future refresh with `merge-digests.mjs` writes to `merged/` by default, so regenerate the narrative parts afterward or pass `--out`.
